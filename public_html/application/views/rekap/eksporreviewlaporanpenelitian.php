<?php
	...
	header("Content-Disposition: attachment; filename=rekap_review_laporan_penelitian_$date.xls");
?>
<h2>Rekap Review Laporan Penelitian</h2>
	...
	<tbody>
		<?php
			...
			foreach($penelitian as $p)
			{
				...
				//Nilai reviewer
					$nilai1 = $this->mrekap->nilailaporanreviewer($p->id_usulan,$reviewer1['user']);
					$hitnilai1 = $this->mrekap->hitnilailaporanreviewer($p->id_usulan,$reviewer1['user']);
					...
					$nilai2 = $this->mrekap->nilailaporanreviewer($p->id_usulan,$reviewer2['user']);
					$hitnilai2 = $this->mrekap->hitnilailaporanreviewer($p->id_usulan,$reviewer2['user']);
					...
					if($hitnilai2>0)
					{
						...
						else
						{
							$skor = $nilai2['skor'];
							$review = explode(',',$skor);
							$skor2 = ((intval($review[0])*10)+(intval($review[1])*20)+(intval($review[2])*20)+(intval($review[3])*20)+(intval($review[4])*30))/7;
						}
					}
				}
				
				//realisasi
				$real = $this->mrekap->realisasijurnalpenelitian($p->id_usulan);
				
				//realisasi
				$realhki = $this->mrekap->realisasihkipenelitian($p->id_usulan);
				
				//relevansi
				$rele = $this->mrekap->relevansipenelitian($p->id_usulan);
				...
				<td>".($hitnilai1>0?$nilai1['hasilreview_laporan']:'')."</td>
				<td>".($hitnilai2>0?$nilai2['hasilreview_laporan']:'')."</td>
				<td>".$p->luaran."</td>
				<td>";
				if(count($real)>0)
