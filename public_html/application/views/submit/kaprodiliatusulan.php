					...
					<div class="row">
						...
						<div class="col-md-8">
							<p>
								...
								$angg = $this->msubmit->perananggota($id_usulan, 'Penelitian');
								$hitpisah = count($angg);

								$anggdosenluar = $this->msubmit->perananggotadosenluar($id_usulan, 'Penelitian');
								$hitsdosenluar = count($anggdosenluar);
								if ($hitpisah > 0 || $hitsdosenluar > 0) {
									echo '<ol style="margin-left:-23px;margin-top:-17px">';
									foreach ($angg as $a) {
										$okdeal = $this->msubmit->cekanggotasetuju($a->id_dosen, $id_usulan);

										if ($okdeal > 0) {
											$setok = '<span class="badge badge-success">Setuju</span>';
										} else {
											$setok = '<span class="badge badge-warning">Belum Respon</span>';
										}
										if (($hitpisah + $hitsdosenluar) == 1) {
											echo $a->namalengkap . ' (' . $setok . ')<br>';
											echo 'Peran : ' . $a->tugas;
										} else {
											echo '<li>' . $a->namalengkap . ' (' . $setok . ')<br>Peran : ' . $a->tugas . '</li>';
										}
									}
									if ($hitsdosenluar > 0) {
									...
									if ($usulan['anggotadosen'] <> '') {
										...
										for ($i = 0; $i < $hit; $i++) {
											//cek persetujuan
											$deal = $this->msubmit->hitanggotausulan($ambil[$i], $this->uri->segment(3));
											$okdeal = $this->msubmit->anggotausulan($ambil[$i], $this->uri->segment(3));
											if ($deal > 0)
												$setuju = $okdeal['setuju'];
											else
												$setuju = 'Belum Respon';
					...
					<div class="row">
						...
						<div class="col-md-8">
							<p><?php
								...
								$angg = $this->msubmit->peranmhs($id_usulan, 'Penelitian');
								$hits = count($angg);
								if ($hits == 0) {
									$angg = $this->msubmit->peranmhsid($id_usulan, 'Penelitian');
									$hits = count($angg);
								}
