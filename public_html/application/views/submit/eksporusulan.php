...
header("Content-Disposition: attachment; filename=data_usulan_penelitian_$date.xls");
?>
<style>
	table {
	...
	<tbody>
		...
		foreach ($usulan as $p) {
			...
			echo "<tr>
						  ...
						  <td>" . $p->skema . "</td>
						  <td>" . $p->sumberdana . "</td>
						  <td>" . @($total['bahan'] + $total['kumpul'] + $total['sewa'] + $total['analis'] + $total['lapor']) . "</td>
						  <td>" . $prodi['prodi'] . "</td>
			...
			echo "</td>	
						  ...
						  <td>" . ($maju <> '' ? '<a href="' . base_url('assets/uploadbox/' . $maju) . '" target="_blank">' . $maju . '</a>' : '-') . "</td>
						  <td>" . ($lap <> '' ? '<a href="' . base_url('assets/uploadbox/' . $lap) . '" target="_blank">' . $lap . '</a>' : '-') . "</td>
