<div class="container-fluid">
		  ...
		  <div class="card shadow mb-4">
            <div class="card-header py-3">
			  ...
			  <?php
				$cekdana = $this->mpengabdian->cekdana($this->uri->segment(3));
				if($total=='')
					$rabnya = 0;
				else
					$rabnya = $total['bahan']+$total['kumpul']+$total['sewa']+$total['analis']+$total['lapor'];
				$hitung = $rabnya-$cekdana['jmldana'];
				if($rabnya < $cekdana['jmldana'])
				{		
					echo '<h6 class="font-weight-bold text-info" style="float:right;margin-top:-20px">Total Dana '.rupiah($rabnya).' | Dana Internal '.rupiah($cekdana['jmldana']).'</h6>';
				}
				elseif($rabnya == $cekdana['jmldana'])
				{		
					echo '<h6 class="font-weight-bold text-success" style="float:right;margin-top:-20px">RAB sudah sesuai Jumlah Dana Internal</h6>';
				}
				else
				{
					echo '<h6 class="font-weight-bold text-info" style="float:right;margin-top:-20px">Total Dana '.rupiah($rabnya).' | Dana Internal '.rupiah($cekdana['jmldana']).' | Dana Mandiri '.rupiah($hitung).'</h6>';
				}
			  ?>
            </div>
            <div class="card-body">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="<?php echo base_url().'pengabdian/simpanrab/'.$this->uri->segment(3); ?>">
...
<script>
    ...
    bootbox.confirm({
		...
		{
			if(result)
			window.location = "<?php echo base_url();?>pengabdian/hapusrab/" + id + "/" + usulan;
		}
	})
	});
