...
<div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Submit Prosiding</h1>
          </div>
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Form Submit Prosiding</h6>
            </div>
            <div class="card-body">
              <form id="tambahjurnal" method="post" action="<?php echo base_url().'dashboard/simpanprosiding'; ?>" enctype="multipart/form-data">
		<div class="row">
          <div class="form-group col-md-6">
				<input type="hidden" id="id_prosiding" name="id">
				<label>Judul Prosiding</label><span id="judul_status" style="float:right;color:red"></span>
				<textarea name="judul" onkeyup="cekjudul();" class="form-control pros1" id="judulpros" placeholder="Judul Prosiding" required></textarea>
				<label>Nama Prosiding</label>
				<input type="text" name="namaprosiding" class="form-control pros2" placeholder="Nama Prosiding" required>
				...
				<label>Volume</label>
				<input type="text" name="volume" class="form-control pros5" placeholder="Volume (jika ada)">
				...
				<label>Nomor</label>
				<input type="text" name="nomor" class="form-control pros6" placeholder="Nomor (jika ada)">
				<label>ISBN/ISSN</label>
				<input type="text" name="isbn" class="form-control pros7" placeholder="ISBN/ISSN" required>
				<label>URL</label>
				<input type="text" name="url" class="form-control pros8" placeholder="http://" required>
				...
				<input type="file" name="fileupload" class="form-control unggah" placeholder="File Prosiding (PDF)" required>
				<label for="recipient-name" class="col-form-label">File Prosiding : 
			<b class="pros10"></b></label>
		  </div>
      </div>
      </div>
...
<script>
	...
	function cekjudul()
	{
		var judul=document.getElementById( "judulpros" ).value;
			
		if(judul)
		{
			$.ajax({
				type: 'post',
				url: "<?php echo base_url('dashboard/cekjudulpros');?>",
				data: {
					judul_pros:judul,
				},
