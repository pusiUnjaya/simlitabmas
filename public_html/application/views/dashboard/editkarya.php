...
<div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Edit Karya Monumental</h1>
          </div>
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Form Edit Karya Monumental</h6>
            </div>
            <div class="card-body">
              <form id="tambahjurnal" method="post" action="<?php echo base_url().'dashboard/simpankarya'; ?>" enctype="multipart/form-data">
		<div class="row">
          <div class="form-group col-md-6">
				<input type="hidden" id="id_karya" value="<?php echo $tambahan['id_karya']; ?>" name="id">
				...
				<select name="jenis" id="selectjenis" class="form-control karya1">
					<?php
						...
						for($i=0;$i<$n;$i++)
						{
							if($tambahan['jenis_karya']==$peran[$i])
				...
				<select name="level" id="selectlevel" class="form-control karya2">
					<?php
						...
						for($i=0;$i<$n;$i++)
						{
							if($tambahan['level_karya']==$peran[$i])
								echo '<option value="'.$peran[$i].'" selected>'.$peran[$i].'</option>';
							else
								echo '<option value="'.$peran[$i].'">'.$peran[$i].'</option>';
						}
			...
			<div class='form-group col-md-5'>
				...
				<textarea name="deskripsi" class="form-control karya4" placeholder="Deskripsi Karya Monumental" required><?php echo $tambahan['deskripsi']; ?></textarea>
				<label>URL</label>
				<input type="text" name="url" value="<?php echo $tambahan['url']; ?>" class="form-control karya5" placeholder="http://" required>
				<label>File Karya Monumental (PDF) maksimal 20MB</label>
				<input type="file" name="fileupload" class="form-control unggah" placeholder="File Karya Monumental (PDF)">
				<label for="recipient-name" class="col-form-label">File Karya Monumental : 
			<b class="karya6"><?php echo $tambahan['dokumen']; ?></b></label>
		  </div>
      </div>
      </div>
