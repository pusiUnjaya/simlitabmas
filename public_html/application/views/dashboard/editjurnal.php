...
<div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Edit Artikel Jurnal</h1>
          </div>
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Form Edit Artikel Jurnal</h6>
            </div>
            <div class="card-body">
              <form id="tambahjurnal" method="post" action="<?php echo base_url().'dashboard/simpanjurnal'; ?>" enctype="multipart/form-data">
		<div class="row">
          <div class="form-group col-md-6">
				<input type="hidden" id="id_jurnal" value="<?php echo $tambahan['id_jurnal']; ?>" name="id">
				<label>Judul Artikel</label>
				<textarea name="judul" id="juduljurnal" class="form-control jurnal1" placeholder="Judul Artikel" required><?php echo ucwords(strtolower($tambahan['judul'])); ?></textarea>
				<label>Nama Jurnal</label>
				<input type="text" name="namajurnal" value="<?php echo $tambahan['nama_jurnal']; ?>" class="form-control jurnal2" placeholder="Nama Jurnal" required>
				...
				<select name="jenispublikasi" id="selectjenis" class="form-control jurnal3">
					<?php
						...
						for($i=0;$i<$n;$i++)
						{
							if($tambahan['jenis_publikasi']==$jenis[$i])
				...
				<select name="statusjurnal" id="selectstatus" class="form-control jurnal4">
					<?php
						...
						for($i=0;$i<$n;$i++)
						{
							if($tambahan['status_jurnal'])
								echo '<option value="'.$statusjurnal[$i].'" selected>'.$statusjurnal[$i].'</option>';
							else
				...
				<input type="text" name="tahun" value="<?php echo $tambahan['tahun_publikasi']; ?>" class="form-control jurnal6" placeholder="Tahun Publikasi" required>
			...
			<div class='form-group col-md-5'>
				...
				<input type="text" name="authorlain" id="authorlain" class="form-control" placeholder="Author Lain Biarkan Kosong Jika tidak ada Perubahan">
					...
					<input type="hidden" value="<?php echo $tambahan['authorlain']; ?>" name="iddosen" id="values">
				<label>Volume</label>
				<input type="text" name="volume" value="<?php echo $tambahan['volume']; ?>" class="form-control jurnal7" placeholder="Volume" required>
				<label>Nomor</label>
				<input type="text" name="nomor" value="<?php echo $tambahan['nomor']; ?>" class="form-control jurnal8" placeholder="Nomor" required>
				<label>Nomor Halaman Awal</label>
				<input type="text" name="awal" value="<?php echo $tambahan['hal_awal']; ?>" class="form-control jurnal9" placeholder="Nomor Halaman Awal" required>
				<label>Nomor Halaman Akhir</label>
				<input type="text" name="akhir" value="<?php echo $tambahan['hal_akhir']; ?>" class="form-control jurnal10" placeholder="Nomor Halaman Akhir" required>
				<label>URL Artikel</label>
				<input type="text" name="url" value="<?php echo $tambahan['url']; ?>" class="form-control jurnal11" placeholder="http://" required>
				<label>E-ISSN/P-ISSN</label>
				<input type="text" name="issn" value="<?php echo $tambahan['issn']; ?>" class="form-control jurnal12" placeholder="ISSN" required>
			...
			<b class="jurnal13"><?php echo $tambahan['file_jurnal']; ?></b></label>
