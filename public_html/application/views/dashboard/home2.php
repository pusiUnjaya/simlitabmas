...
<div class="container-fluid">
						...
						<?php
							...
							foreach($riwayat as $p)
							{
								...
								elseif($this->session->userdata('sesi_id')<>$p->pengusul)
								{
									...
									$peranku = '';
								echo "<tr>
										  <td width='70%'><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_laporan_akhir."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
							...
							{
								$tahun = date('Y',strtotime($p->modified));
								if(($tahun<2023 && $p->validasi==0) || ($tahun>=2023 && $p->validasi==1))
								{
									...
									elseif($this->session->userdata('sesi_dosen')<>$p->ketua)
									{
										...
										$peranku = '';
									echo "<tr>
											  <td><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_laporan_akhir."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
						...
						<?php
							...
							foreach($tambahanpkm as $p)
							{
								...
								elseif($this->session->userdata('sesi_dosen')<>$p->ketua)
								{
									...
									$peranku = '';
								echo "<tr>
										  <td><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->filelaporan."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
										  <br><b>Tahun : </b>".$p->tahun." | <b>Peran : </b>".$peranku." | <b>Sumber Dana :</b> 
										  ".$p->sumberdana."
					  ...
					  <tbody>
						<?php
							...
							foreach($listjurnal as $p)
							{
								...
								//luaran dari luar prosedur simlitabmas
								...
								$tapub = intval($p->tahun_publikasi);

								if(($tapub>=2023 && $p->catatan<>'' && $p->validasi=='0') || ($tapub>=2023 && $p->catatan=='' && $p->validasi=='0'))
									$catat = "class='table-danger'";
								else
									$catat = "";
								 
								if(($tapub>=2023 && $p->validasi==1 && $p->catatan=='') || ($this->session->userdata('sesi_id')==$p->user && $tapub>=2023 && ($p->validasi==1 || $p->validasi==0)))
								{
									echo "<tr ".$catat.">
										  <td width='70%'><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_jurnal."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>";
								if($p->usulan<>'' && $p->sbgluaran<>'')
									echo "<br>Sebagai : ".$p->sbgluaran;
								echo "<br><b>Jurnal : </b>".$p->nama_jurnal." | <b>Peran</b> : ".$peran."
										  <br><b>Tahun : </b>".$p->tahun_publikasi." | 
										  <b>Volume : </b>".$p->volume." | 
										  <b>ISSN : </b>".$p->issn."
										  <br><b>URL : </b><a href='".$p->url."' target='_blank'>".$p->url."</a>";
								echo $p->catatan==""?"":"<br><b style='color:red'>Catatan : ".$p->catatan."</b>";
								echo "</td><td>".$p->tahun_publikasi."</td><td width='12%'>";
								if($this->session->userdata('sesi_id')==$p->user)
								{
									echo "<a href='".base_url()."dashboard/editjurnal/".$p->id_jurnal."' class='shadow-sm' title='Edit Jurnal'><i class='fas fa-edit fa-sm'></i></a>
								<a href='#' data-id='".$p->id_jurnal."' class='shadow-sm hapusjurnal' title='Hapus Jurnal'><i class='fas fa-trash fa-sm'></i></a>";
								}
								echo "</td>
										</tr>";
								$no++;
								...
								//luaran dari prosedur simlitabmas
								if($p->usulan<>''){
								$penulisnya = $this->msubmitpribadi->caripenulisjurnal($p->usulan);
								$hitpenulisnya = $this->msubmitpribadi->hitpenulisjurnal($p->usulan);
								
								$penulispkm = $this->msubmitpribadi->caripenulisjurnalpkm($p->usulan);
								$hitpenulispkm = $this->msubmitpribadi->hitpenulisjurnalpkm($p->usulan);
								...
								if($p->sbgluaran=="Luaran Penelitian" && $hitpenulisnya>0 && $penulisnya['anggotadosen']<>''){
								
								// echo $this->db->last_query();exit;
								
								$listw = explode(',',($penulisnya['authorlain']==''?$penulisnya['anggotadosen']:$penulisnya['authorlain']));
								
								if(in_array($this->session->userdata('sesi_dosen'),$listw) || $this->session->userdata('sesi_id')==$penulisnya['pengusul'])
								{
									echo "<tr>
										  <td><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_jurnal."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
										  <br><b>Jurnal : </b>".$p->nama_jurnal." | <b>Peran</b> : ".$peran."
										  <br><b>Tahun : </b>".$p->tahun_publikasi." | 
										  <b>Volume : </b>".$p->volume." | 
										  <b>ISSN : </b>".$p->issn."
										  <br><b>URL : </b><a href='".$p->url."' target='_blank'>".$p->url."</a>
										  </td><td>".$p->tahun_publikasi."</td><td></td>
										</tr>";
										$no++;
								}
								else
								{
									if($this->session->userdata('sesi_id')==$penulisnya['pengusul'])
									{
										echo "<tr>
											  <td>".$no."</td>
											  <td><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_jurnal."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
								...
								if($p->sbgluaran=="Luaran Pengabdian" && $hitpenulispkm>0 && $penulispkm['anggotadosen']<>''){
								
								// echo $this->db->last_query();exit;
								
								$listw = explode(',',$penulispkm['anggotadosen']);
								
								if(in_array($this->session->userdata('sesi_dosen'),$listw) || $this->session->userdata('sesi_id')==$penulispkm['pengusul'])
								{
									echo "<tr>
										  <td><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_jurnal."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
										  <br><b>Jurnal : </b>".$p->nama_jurnal." | <b>Peran</b> : ".$peran."
								...
								{
									if($this->session->userdata('sesi_id')==$penulispkm['pengusul'])
									{
										echo "<tr>
											  <td><a href='' title='Lihat Dokumen' data-toggle='modal' data-filenya='".$p->file_jurnal."' data-jenisfile='File Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
											  <br><b>Jurnal : </b>".$p->nama_jurnal." | <b>Peran</b> : ".$peran."
					  ...
					  <tbody>
						<?php
							...
							foreach($listhki as $p)
							{
								...
								$tapub = intval($p->tahun_pelaksanaan);
								...
								echo "<tr ".$catat.">
										  <td width='70%'><a href='' data-toggle='modal' data-filenya='".$p->file_hki."' data-jenisfile='File Dokumen' title='Lihat Dokumen' data-target='#liatfile'>".ucwords(strtolower($p->judul))."</a>
								...
								echo '<br>Author : '.$ketua['namalengkap'];
								echo '<br>Author Lain : '.$namauthor;
								echo '<br><b>Sebagai Luaran : '.$p->sbgluaran.'</b>';
								echo $p->catatan==""?"":"<br><b style='color:red'>Catatan : ".$p->catatan."</b>";
								echo "</td>
								...
								if($this->session->userdata('sesi_id')==$p->user)
								{
									echo "<a href='".base_url()."dashboard/edithki/".$p->id_hki."' class='shadow-sm' title='Edit HKI'><i class='fas fa-edit fa-sm'></i></a>
								<a href='#' data-id='".$p->id_hki."' class='shadow-sm hapushki' title='Hapus HKI'><i class='fas fa-trash fa-sm'></i></a>";
								}
								echo "</td></tr>";
								$no++;
								}
					  ...
					  <tbody>
						<?php
							foreach($listpros as $p)
							{
								...
								{
									echo "<tr>
										  <td width='70%'>".ucwords(strtolower($p->judul))."</td>
										  <td>".$p->tahun."</td>
										  <td><a href='' data-toggle='modal' data-filenya='".$p->file_prosiding."' data-jenisfile='File Dokumen' data-target='#liatfile'>Lihat Dokumen</a></td><td width='12%'>";
									if($this->session->userdata('sesi_id')==$p->user)
									{
										 echo "<a href='".base_url()."dashboard/editprosiding/".$p->id_prosiding."' class='shadow-sm' title='Edit Prosiding'><i class='fas fa-edit fa-sm'></i></a>
									 <a href='#' data-id='".$p->id_prosiding."' class='shadow-sm hapusprosiding' title='Hapus Prosiding'><i class='fas fa-trash fa-sm'></i></a>";
									}
									echo "</td></tr>";
								}
							}
							...
							foreach($listbuku as $p)
							{
								...
								if($this->session->userdata('sesi_id')==$p->user || in_array($this->session->userdata('sesi_dosen'),$list))
								{
									echo "<tr>
										  ...
										  <td><a href='' data-toggle='modal' data-filenya='".$p->file_buku."' data-jenisfile='File Dokumen' data-target='#liatfile'>Lihat Dokumen</a></td><td width='12%'>";
									if($this->session->userdata('sesi_id')==$p->user)
									{
										echo "<a href='".base_url()."dashboard/editbuku/".$p->id_buku."' class='shadow-sm' title='Edit Buku'><i class='fas fa-edit fa-sm'></i></a>
										<a href='#' data-id='".$p->id_buku."' class='shadow-sm hapusbuku' title='Hapus Buku'><i class='fas fa-trash fa-sm'></i></a>";
									}
									echo "</td></tr>";
								}
							}
			  ...
			  <div class="card shadow mb-4">
				...
				<div class="card-body">
					<table class="table table-bordered display" id="" width="100%" cellspacing="0">
					  <thead>
						<tr>
						  <th width="70%">Jenis Karya Monumental</th>
						  <th>Tahun</th>
						...
						<?php
							foreach($listkarya as $p)
							{
								...
								{	
									...
									if($this->session->userdata('sesi_id')==$p->user)
									{
										echo "<a href='".base_url()."dashboard/editkarya/".$p->id_karya."' class='shadow-sm' title='Edit Karya Monumental'><i class='fas fa-edit fa-sm'></i></a>
										<a href='#' data-id='".$p->id_karya."' class='shadow-sm hapuskarya' title='Hapus Karya Monumental'><i class='fas fa-trash fa-sm'></i></a>";
									}
									echo "</td></tr>";
								}
							}
						...
						<?php
							foreach($listnaskah as $p)
							{
								...
								if($this->session->userdata('sesi_id')==$p->user || in_array($this->session->userdata('sesi_dosen'),$list))
								{
									...
									if($this->session->userdata('sesi_id')==$p->user)
									{
										echo "<a href='".base_url()."dashboard/editnaskah/".$p->id_naskah."' class='shadow-sm' title='Edit Naskah Akademik/Urgensi'><i class='fas fa-edit fa-sm'></i></a>
										<a href='#' data-id='".$p->id_naskah."' class='shadow-sm hapusnaskah' title='Hapus Naskah Akademik/Urgensi'><i class='fas fa-trash fa-sm'></i></a>";
									}
									echo "</td></tr>";
								}
							}
          ...
          <div class="row">
			...
			<div class="col-xl-8 col-lg-7">
              <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Buka Tutup Pendaftaran Usulan Baru</h6>
                  <div class="dropdown no-arrow">
                ...
                <div class="row">
					          <div class="col-lg-6 mb-6">
					          	<form id="bukatutup" method="post" action="<?php echo base_url().'dashboard/bukatutup'; ?>">
					          		<div class="form-group id_3">
									  			<label>Waktu Buka Pendaftaran</label>
					                <input type="text" name="buka" value="<?php echo $bukaan['tglbuka']; ?>" class="form-control" placeholder="Tanggal Buka Pendaftaran" required>
					              </div>
												<div class="form-group id_3">
									  			<label>Waktu Tutup Pendaftaran</label>
					                <input type="text" name="tutup" class="form-control" value="<?php echo $bukaan['tgltutup']; ?>" placeholder="Tanggal Tutup Pendaftaran" required>
					              </div>
					          ...
					          <div class="col-lg-6 mb-6">
					          	<?php
					          		...
					          		{
					          			$warna = 'bg-danger';
					          			$teks = 'Pendaftaran Tertutup';
					          			$durasi = '-';
					          		}
					          		else
					          		{
					          			$warna = 'bg-success';
					          			$teks = 'Pendaftaran Terbuka';
					          			$durasi = tgl_indo($bukaan['tglbuka'],1).' s/d '.tgl_indo($bukaan['tgltutup'],1);
					          		}
					          	?>
					          	<div class="card <?php echo $warna; ?> text-white shadow">
												<div class="card-body">
													<?php echo $teks; ?>
													<div class="text-white small"><?php echo $durasi; ?></div>
												</div>
											</div>
					          </div>
					      </div>
